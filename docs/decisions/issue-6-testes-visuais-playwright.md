...
## PR REVIEW (chatgpt-codex-connector, 2ª rodada) → FIX — 2026-09-13

- Decisão: mudar `reuseExistingServer` de `!process.env.CI` para `false` em resposta ao achado P2 de follow-up no commit d510927 — com reuso habilitado, um `vite dev` que o desenvolvedor já tivesse de pé na porta 5173 (outro terminal) era reaproveitado sem o `webServer.env`, reabrindo o mesmo vazamento de origem já corrigido para o caso "sem servidor prévio". Reproduzido subindo manualmente um `vite dev` na 5173 com `VITE_API_BASE_URL=http://localhost:9999` antes do e2e: o teste reaproveitava esse servidor e travava por timeout; com o fix, o mesmo servidor "rogue" no ar faz a suíte falhar alto ("port already in use") em vez de silenciosamente usar a origem errada.
- Decisão: aceitar o trade-off de nunca reaproveitar servidor local (recomeça o `vite dev` a cada `npm run test:e2e`) em favor de reprodutibilidade garantida (RNF04) — o custo é ~1-2s de startup a mais por execução local, sem efeito no CI (que já não reaproveitava).
- Risco aceito: nenhum — comportamento verificado nos dois sentidos (com o servidor "rogue" no ar, antes e depois do fix) e suíte completa (lint, coverage, build, e2e) permanece verde.
